...
1. Would you say this data is in tidy format?
   ...
   Depends on my question. If I want to compare across all three variable, yes. If I just want to compare how each how words are changing per Film, I would want to pivot_wider to have one row for each Film.

2. Widen the data so that we see the words spoken by each race, by putting race as its own column.

```{r}
lotr_wide <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), #keep everything but Race and Words
              names_from = Race, #don't need "" when calling
              values_from = Words)
lotr_wide
```
...
```{r}
lotr_wide %>% 
  pivot_longer(cols = c(Elf, Hobbit, Man), #columns going into new column
               names_to  = "Race", #need "" when making
               values_to = "Words") %>% 
  print()
```
...
```{r}
guest_long <- guest %>% 
  pivot_longer(cols = c(-name, -party), 
