...
select(mtcars, mpg, cyl, disp, hp)
# Better way:
select(gapminder, country:lifeExp)
select(mtcars, mpg:hp)
# select(gapminder, 1:4) don't iuse numbers when you don't have to
```
...
select(mtcars, milespergallon = mpg, everything())
...
select(gapminder, year, lifeExp, country) %>% #makes previous the first argument in the next
...
```{r}
gapminder %>%
# filter(pop > 100000000, continent == "Asia", country == "Brazil" | country == "China")
  filter(pop > 100000000, continent == "Asia", country %in% c("Brazil", "China"))

# == would recycle, compare 1 to Brazil, then 2 to China, then 3 to Brazil again
# %in% compares each to Brazil or China
```
...
```{r}
gapminder %>%
  mutate(cc = (country  & year == 1952, 70, lifeExp)) %>%      filter(country == "Canada") 
```
