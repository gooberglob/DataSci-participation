...
## Exercise 1: Overlapping Points:DONE
...
## Exercise 2: Line for each Country:DONE
...
## Exercise 3: More gdpPercap vs lifeExp:DONE
...
# Am I allowed to change the figure width? I can't figure out how else to make the x axis scale fit
gapminder %>% 
  filter(continent != "Oceania") %>% 
  ggplot(aes(x = gdpPercap, y = lifeExp, size = pop)) + #added size argument #teacher had `fill = continent`
  ylab("Life Expectancy") + #changed y axis label
  facet_wrap(~ continent, nrow = 1, scales = "free_x") + #nrow to 1, scales to free
  geom_point(shape = 21, fill = "seagreen2", color = "grey53", alpha = 0.2) + #added color and shape
  theme_bw() + #theme for fun
  scale_x_log10("GPD Per Capita", labels = scales::dollar_format()) +
  scale_size_area(name = "Population", labels = scales::comma_format()) #legend(Population) control #+ 
# scale_fill_discrete(guide = FALSE) (teacher had this, but didn't change anything for me)
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) + # aes argument should be set for ggplot, not for geom_point, not sure why
  geom_point(size = .1) +
  scale_x_log10(labels = scales::dollar_format())
```


## Exercise 4: Walking caribou: DONE
...
tribble(
  ...
  ggplot(aes(x, y)) + 
  geom_path(arrow = arrow()) + #change geom_line to geom_path, add arrow
  geom_text(aes(label = time)) 
```

## Exercise 5: Life expectancies in Africa

### 5(a) Unhiding the data:DONE
...
gapminder %>% 
  ...
  geom_point(alpha = 0.25, height = 0.2) +
...
# filter(continent == "Americas") %>% 
# ggplot(aes(country, lifeExp)) + 
#  ggridges::geom_density_ridges(aes(group = country))
...
## Exercise 6: Bar plot madness:DONE
...
mtcars %>% 
  ...
  ggplot(aes(cyl, ..prop..)) + #not sure what ..prop.. is doing
  geom_bar(aes(fill = transmission), position = "dodge") + #change color to fill
  scale_x_continuous(breaks = c(4, 6, 8)) #add scale
```
